...
static ComparisonMetrics run_rl(GeoBox& gb, PolicyMode m) {
    EpisodeConfig cfg = regression_config();
    bid_policy(BidPolicyKind::MAPPO).clear_buffers();
    EpisodeRunner runner(cfg, gb, 7u);
...
bool run_regression_tests(const std::string& osm_file, const std::string& cache_dir)
{
    ...
    // ── [2] Scenario grid (3 task × 3 congestion × 3 fleet = 27) ──────────────
    const auto grid = make_scenario_grid();
    CHECK(grid.size() == 27, "scenario grid size drifted");
    std::cout << "  [2] Scenario grid OK — 27 combos\n";
    ...
    auto check_rl = [&](const char* label, PolicyMode m,
        ...
        const auto a = run_rl(gb, m);
        const auto b = run_rl(gb, m);
        ...
        // Observed values printed BEFORE the golden asserts so a drift run
        // directly provides the new baselines.
        std::cout << "  [3] RL " << label << " observed — appeared=" << a.tasks_appeared
                  << " completed=" << a.tasks_completed
                  << " thr=" << a.throughput_rate
                  << " cong=" << a.mean_congestion << "\n";
    ...
    auto check_sota = [&](const char* label, ISolver& s, int g_app, int g_comp, float g_thr) {
        EpisodeConfig cfg = regression_config();
        SolverRunner r1(cfg, gb, {}, kSeed); const auto a = r1.run(s);
        SolverRunner r2(cfg, gb, {}, kSeed); const auto b = r2.run(s);
        CHECK(a.tasks_appeared == b.tasks_appeared && a.tasks_completed == b.tasks_completed,
        ...
        std::cout << "  [4] SoTA " << label << " observed — appeared=" << a.tasks_appeared
                  << " completed=" << a.tasks_completed
                  << " thr=" << a.throughput_rate << "\n";
