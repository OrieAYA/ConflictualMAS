...
//
// COMPARISON AXIS:
//   TokenPassing     — static A* + argmin distance         (no congestion)
//   CongestionAware  — static A* + argmin BPR pickup       (decision-only)
//   FaithfulCA       — BPR-A*    + γ-weighted full trip    (decision + routing)
//   TrafficFlow      — UE guide  + lex contraflow          (Phase 4)
...
//   - NO dependency on PathHelper (FaithfulCA computes its
//     own BPR-aware paths; PathHelper handles static A*).
