...
private:
    ...
    struct AgentEntry {
        float pickup_cost   = kNoContact;   // static distance from pickup side (m)
        float delivery_cost = kNoContact;   // static distance from delivery side (m)
    };
    ...
    // Rebuild candidates_ from matrix_: any agent reachable from at least one
    // side with a valid plan order, sorted ascending by combined cost,
    // capped at max_candidates.
    void collect_candidates(PDPGlobalMemory& memory);

    // Pickup strictly before delivery in the agent's plan (or neither).
