...
// ════════════════════════════════════════════════════════════════════════════
//
// Runs one training/evaluation episode end-to-end: generate the ScheduledTask
// stream (EpisodeGenerator), then per step inject arrivals, offer them to agents
// (TAM + policy), and advance each agent edge-by-edge along the road graph. In
// train_mode, MAPPO experiences are buffered and train_epoch() runs at episode
// end. One instance = one episode; the caller reuses the GeoBox/Pathfinder.
//
// PolicyMode selects how a task is accepted/scored. Random and Greedy do NOT
// write to the training buffer — safe as baselines without contaminating MAPPO.
